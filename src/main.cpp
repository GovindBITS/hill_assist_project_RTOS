...
constexpr uint16_t kMotorTaskStackWords = 768;
constexpr uint16_t kBrakeTaskStackWords = 512;
constexpr uint16_t kImuTaskStackWords = 768;
constexpr uint16_t kDisplayTaskStackWords = 512;
constexpr uint16_t kCommTaskStackWords = 768;

constexpr UBaseType_t kMotorTaskPriority = tskIDLE_PRIORITY + 4;
constexpr UBaseType_t kBrakeTaskPriority = tskIDLE_PRIORITY + 4;
constexpr UBaseType_t kImuTaskPriority = tskIDLE_PRIORITY + 3;
constexpr UBaseType_t kCommTaskPriority = tskIDLE_PRIORITY + 3;
...
TaskHandle_t gMotorTaskHandle = nullptr;
TaskHandle_t gBrakeTaskHandle = nullptr;
TaskHandle_t gImuTaskHandle = nullptr;
TaskHandle_t gDisplayTaskHandle = nullptr;
TaskHandle_t gCommTaskHandle = nullptr;

QueueHandle_t gMotorCommandQueue = nullptr;
QueueHandle_t gBrakeCommandQueue = nullptr;

SemaphoreHandle_t gImuDataMutex = nullptr;
...
void updateDisplay(const ImuSample& sample, const LevelState& state);
void logImuToSerial(const ImuSample& sample, const LevelState& state, uint32_t nowMs);
...
bool enqueueMotorCommand(int16_t left, int16_t right);
bool snapshotImuState(ImuSample& sample, LevelState& state);
void MotorControlTask(void* param);
void BrakeControlTask(void* param);
...
void driveBrakeServos(bool engage) {
...
void dispatchBrakeCommand(BrakeCommand cmd) {
  if (gBrakeCommandQueue == nullptr || gBrakeTaskHandle == nullptr) {
    driveBrakeServos(cmd == BrakeCommand::Apply);
    return;
  }

  if (xQueueSend(gBrakeCommandQueue, &cmd, 0) != pdPASS) {
    BrakeCommand discarded;
    if (xQueueReceive(gBrakeCommandQueue, &discarded, 0) == pdPASS) {
      xQueueSend(gBrakeCommandQueue, &cmd, 0);
    }
  }
}

void applyBrakes() {
  dispatchBrakeCommand(BrakeCommand::Apply);
}

void releaseBrakes() {
  dispatchBrakeCommand(BrakeCommand::Release);
}
...
void setup() {
  ...
  gImuDataMutex = xSemaphoreCreateMutex();
  if (gImuDataMutex == nullptr) {
    Serial.println(F("[rtos] Failed to create IMU data mutex."));
  }

  gMotorCommandQueue = xQueueCreate(4, sizeof(MotorCommand));
  if (gMotorCommandQueue == nullptr) {
    Serial.println(F("[rtos] Failed to create motor queue."));
  } else {
    enqueueMotorCommand(gCommandLeft, gCommandRight);
  }

  gBrakeCommandQueue = xQueueCreate(6, sizeof(BrakeCommand));
  if (gBrakeCommandQueue == nullptr) {
    Serial.println(F("[rtos] Failed to create brake queue."));
  }

  BaseType_t createStatus = pdPASS;
  createStatus &= xTaskCreate(MotorControlTask,
                              "motor",
                              kMotorTaskStackWords,
                              nullptr,
                              kMotorTaskPriority,
                              &gMotorTaskHandle);
  createStatus &= xTaskCreate(BrakeControlTask,
                              "brake",
                              kBrakeTaskStackWords,
                              nullptr,
                              kBrakeTaskPriority,
                              &gBrakeTaskHandle);
  createStatus &= xTaskCreate(ImuTask,
                              "imu",
                              kImuTaskStackWords,
                              nullptr,
                              kImuTaskPriority,
                              &gImuTaskHandle);
  createStatus &= xTaskCreate(DisplayTask,
                              "display",
                              kDisplayTaskStackWords,
                              nullptr,
                              kDisplayTaskPriority,
                              &gDisplayTaskHandle);
  createStatus &= xTaskCreate(CommunicationTask,
                              "comm",
                              kCommTaskStackWords,
                              nullptr,
                              kCommTaskPriority,
  ...
  if (createStatus != pdPASS) {
    Serial.println(F("[rtos] Failed to create one or more tasks."));
  } else {
    Serial.println(F("[rtos] Scheduler starting."));
    vTaskStartScheduler();
  }

  Serial.println(F("[rtos] Scheduler unavailable, entering safe loop."));
  while (true) {
    delay(1000);
  }
}
...
void MotorControlTask(void* param) {
  ...
  for (;;) {
    if (gMotorCommandQueue != nullptr) {
      while (xQueueReceive(gMotorCommandQueue, &cmd, 0) == pdPASS) {
        setMotorTargets(cmd.left, cmd.right);
      }
    }
    serviceMotorOutputs();
...
void BrakeControlTask(void* param) {
  ...
  BrakeCommand request{};
  (void)param;

  for (;;) {
    if (gBrakeCommandQueue != nullptr) {
      while (xQueueReceive(gBrakeCommandQueue, &request, 0) == pdPASS) {
        driveBrakeServos(request == BrakeCommand::Apply);
      }
    }
    updateButton(millis());
    vTaskDelay(delayTicks);
...
void ImuTask(void* param) {
  ...
  for (;;) {
    ...
    if (gImuReady && (now - gLastImuReadMs) >= kImuReadIntervalMs) {
      gLastImuReadMs = now;
      ImuSample sample;
      if (readMpu9250(sample)) {
        ...
        sample.valid = true;
        if (gImuDataMutex != nullptr) {
          if (xSemaphoreTake(gImuDataMutex, portMAX_DELAY) == pdPASS) {
            gImuSample = sample;
            updateLevelState(gImuSample);
            xSemaphoreGive(gImuDataMutex);
          }
        } else {
          gImuSample = sample;
          updateLevelState(gImuSample);
...
void DisplayTask(void* param) {
  ...
  for (;;) {
    const uint32_t now = millis();
    if (gDisplayReady && (now - gLastDisplayRefreshMs) >= kDisplayRefreshIntervalMs) {
      gLastDisplayRefreshMs = now;
      ImuSample sample{};
      LevelState state{};
      if (snapshotImuState(sample, state)) {
        updateDisplay(sample, state);
      }
    }
    vTaskDelay(delayTicks);
  }
...
void CommunicationTask(void* param) {
  ...
  for (;;) {
    ...
    if ((now - gLastSerialLogMs) >= kSerialLogIntervalMs) {
      ImuSample sample{};
      LevelState state{};
      if (snapshotImuState(sample, state) && sample.valid) {
        gLastSerialLogMs = now;
        logImuToSerial(sample, state, now);
      }
    }
...
void processEspLine(const char* line) {
  ...
  if (strncmp(line, "FWD", 3) == 0) {
    ...
    enqueueMotorCommand(speed, speed);
    return;
  }
  ...
  if (strncmp(line, "MOTORS", 6) == 0 || strncmp(line, "DRV", 3) == 0) {
    ...
    enqueueMotorCommand(static_cast<int16_t>(left), static_cast<int16_t>(right));
    return;
  }
...
bool enqueueMotorCommand(int16_t left, int16_t right) {
  if (gMotorCommandQueue == nullptr || gMotorTaskHandle == nullptr) {
    setMotorTargets(left, right);
    return true;
  }

  const MotorCommand cmd{left, right};
  if (xQueueSend(gMotorCommandQueue, &cmd, 0) != pdPASS) {
    MotorCommand discarded;
    if (xQueueReceive(gMotorCommandQueue, &discarded, 0) == pdPASS) {
      xQueueSend(gMotorCommandQueue, &cmd, 0);
    }
  }
  return true;
}
...
bool snapshotImuState(ImuSample& sample, LevelState& state) {
  ...
  if (xSemaphoreTake(gImuDataMutex, pdMS_TO_TICKS(5)) != pdPASS) {
    return false;
  }
  sample = gImuSample;
  state = gLevelState;
...
void updateDisplay(const ImuSample& sample, const LevelState& state) {
  ...
  if (!gImuReady) {
  ...
  } else if (!state.valid || state.profile == nullptr) {
    display.println(F("MPU9250 ready"));
    display.println(F("Stabilising..."));
  } else {
    display.print(state.profile->orientationLabel);
    display.setCursor(display.width() - 48, 0);
    display.print(F("dT:"));
    display.print(sample.tempC, 1);

    display.setCursor(0, 8);
    display.print(state.profile->axisX.label);
    display.print(':');
    display.print(state.axisXDeg, 1);
    display.setCursor(52, 8);
    display.print(state.profile->axisY.label);
    display.print(':');
    display.print(state.axisYDeg, 1);
    display.setCursor(96, 8);
    if (fabsf(state.axisXDeg) <= kLevelThresholdDeg &&
        fabsf(state.axisYDeg) <= kLevelThresholdDeg) {
    ...
    const int16_t bubblePixelX =
        frameCenterX + static_cast<int16_t>(state.bubbleX * bubbleMaxX);
    const int16_t bubblePixelY =
        frameCenterY + static_cast<int16_t>(state.bubbleY * bubbleMaxY);
...
void logImuToSerial(const ImuSample& sample, const LevelState& state, uint32_t nowMs) {
  ...
  if (state.valid && state.profile) {
    Serial.print(F(" ref="));
    Serial.print(state.profile->orientationLabel);
    Serial.print(F(" "));
    Serial.print(state.profile->axisX.label);
    Serial.print('=');
    Serial.print(state.axisXDeg, 1);
    Serial.print(F("deg "));
    Serial.print(state.profile->axisY.label);
    Serial.print('=');
    Serial.print(state.axisYDeg, 1);
    Serial.print(F("deg"));
  }
  Serial.print(F(" btn="));
  ...
  sendEspTelemetry(sample, state, nowMs);
  sendEspStatus(nowMs);
}
